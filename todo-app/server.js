...
async function go() {
  let client = new MongoClient("mongodb://root:root@localhost/TodoApp?&authSource=admin");
...
app.post("/delete-item", async function (req, res) {
  ...
  await db.collection("items").findOneAndDelete({ _id: new ObjectId(req.body.id) });
  res.send("Success");
});
